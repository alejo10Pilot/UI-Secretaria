...
def obtener_datos_cliente(id_cliente):
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute("SELECT saldo_restante, pago_hasta FROM clientes WHERE id_cliente = %s", (id_cliente,))
...
def obtener_nombre_cliente(id_cliente):
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute("SELECT nombre_cliente FROM clientes WHERE id_cliente = %s", (id_cliente,))
    resultado = cursor.fetchone()
    conn.close()
    if resultado:
        return resultado[0]
...
def registrar_interes(datos):
    print(">> DAO: registrar_interes recibidos:", datos)
    conn = conectar()
    cursor = conn.cursor()

    cursor.execute("""
        INSERT INTO recibos (numero_recibo, id_cliente, fecha_pago, pago_hasta, abono_intereses, saldo_restante, tipo, observaciones, consigno_a)
...
def registrar_abono(datos):
    ...
    cursor.execute("""
        INSERT INTO recibos (numero_recibo, id_cliente, fecha_pago, abono_capital, saldo_restante, tipo, observaciones, consigno_a)
...
def obtener_ultimo_pago_hasta_por_interes(id_cliente):
    ...
    if resultado and resultado[0]:
        ...
        raise ValueError("El cliente no tiene fecha de pago_hasta definida.")